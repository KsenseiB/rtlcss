...
module.exports = {
  ...
  directives: {
    ...
    value: [
      ...
      },
      {
        name: 'append',
        action (decl, expr, context) {
          decl.value = decl.raws.value.raw = decl.raws.value.raw.replace(expr, (match, value) => match + value)
          return true
        }
      },
      {
        name: 'insert',
        action (decl, expr, context) {
          decl.value = decl.raws.value.raw = decl.raws.value.raw.replace(expr, (match, value) => value + match)
          return true
        }
      },
      {
  ...
  processors: [
    ...
    },
    {
      ...
      flip (value) {
        return value.replace(this.cache.replace, (s, m) => s.replace(m, m.replace(this.cache.e, '*').replace(this.cache.w, 'e').replace(this.cache.star, 'w')))
      },
      action (prop, value, context) {
        if (this.cache === null) {
          this.cache = {
