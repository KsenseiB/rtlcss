...
module.exports = {
  ...
  directives: {
    ...
    value: [
      ...
      },
      {
        ...
        action: function (decl, expr, context) {
          let prefix = ''
          decl.raws.value.raw.replace(expr, (m, v) => {
            prefix += v
          })
      ...
      },
      {
        name: 'append',
        action: function (decl, expr, context) {
          decl.value = decl.raws.value.raw = decl.raws.value.raw.replace(expr, (match, value) => {
            return match + value
          })
          return true
        }
      ...
      },
      {
        name: '',
        action: function (decl, expr, context) {
          decl.raws.value.raw.replace(expr, (match, value) => {
  ...
  processors: [
    ...
    },
    {
      ...
      expr: /left/im,
      action: function (prop, value, context) {
        return { prop: prop.replace(this.expr, () => { return 'right' }), value: value }
      }
    },
    {
      name: 'right',
      expr: /right/im,
      action: function (prop, value, context) {
        return { prop: prop.replace(this.expr, () => { return 'left' }), value: value }
      }
    },
    {
      ...
      action: function (prop, value, context) {
        ...
        if (result && result.length === 4 && (state.store.length > 0 || result[1] !== result[3])) {
          let i = 0
          state.value = state.value.replace(this.cache.match, () => {
            return result[(4 - i++) % 4]
          })
        }
    ...
    },
    {
        ...
        if (parts) {
          switch (parts.length) {
            case 2:
              i = 1
              if (parts[0] !== parts[1]) {
                value = value.replace(this.cache.match, () => {
                  return parts[i--]
                })
              }
              break
            case 3:
              ...
              value = value.replace(this.cache.white, m => {
                return m + parts[1] + ' '
              })
              break
            case 4:
      ...
      action: function (prop, value, context) {
        ...
        const state = context.util.guardFunctions(value)
        state.value = state.value.replace(this.cache.slash, m => {
          return this.flip(m)
        })
        return { prop: prop, value: context.util.unguardFunctions(state) }
      }
    },
    {
      ...
      action: function (prop, value, context) {
        ...
        funcSafe.value = funcSafe.value.replace(this.cache.replace, m => { return context.util.negate(m) })
      ...
      action: function (prop, value, context) {
        ...
        if (value.match(this.cache.xKeyword)) {
          ...
          if (parts && parts.length > 0) {
            ...
            state.value = state.value.replace(this.cache.match, () => { return parts.shift() })
    ...
    },
    {
      ...
      flip: function (value, process, context) {
        let i = 0
        return value.replace(this.cache.unit, num => {
