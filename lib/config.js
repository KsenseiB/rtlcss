...
function main (opts, plugins, hooks) {
  ...
  if (Array.isArray(config.stringMap)) {
    ...
    config.stringMap.sort((a, b) => { return a.priority - b.priority })
  }
  ...
  if (Array.isArray(plugins)) {
    if (!plugins.some(plugin => { return plugin.name === 'rtlcss' })) {
  ...
  config.plugins.sort((a, b) => { return a.priority - b.priority })
