...
module.exports = {
  ...
  negate (value) {
    ...
    state.value = state.value.replace(REGEX_NEGATE_ONE, num => REGEX_TOKEN_REPLACEMENT.test(num) ? num.replace(REGEX_TOKEN_REPLACEMENT, m => '(-1*' + m + ')') : parseFloat(num) * -1)
    return this.restoreTokens(state)
  },
  negateAll (value) {
    const state = this.saveTokens(value)
    state.value = state.value.replace(REGEX_NEGATE_ALL, num => REGEX_TOKEN_REPLACEMENT.test(num) ? num.replace(REGEX_TOKEN_REPLACEMENT, m => '(-1*' + m + ')') : parseFloat(num) * -1)
    return this.restoreTokens(state)
  },
  complement (value) {
    const state = this.saveTokens(value)
    state.value = state.value.replace(REGEX_COMPLEMENT, num => REGEX_TOKEN_REPLACEMENT.test(num) ? num.replace(REGEX_TOKEN_REPLACEMENT, m => '(100% - ' + m + ')') : 100 - parseFloat(num))
    return this.restoreTokens(state)
  },
  flipLength (value) {
  ...
  guard (what, who, indexed) {
    ...
    if (state.indexed === true) {
      while (what.test(state.value)) {
        state.value = state.value.replace(what, m => { state.store.push(m); return state.token + ':' + state.store.length + CHAR_TOKEN_END })
      }
    } else {
      state.value = state.value.replace(what, m => { state.store.push(m); return state.token + CHAR_TOKEN_END })
    }
    return state
  },
