...
module.exports = {
  ...
  swap (value, a, b, options) {
    ...
    return value.replace(new RegExp(expr, flags), m => { return compare(m, a, options.ignoreCase) ? b : a })
  },
  swapLeftRight (value) {
  ...
  negate (value) {
    ...
    state.value = state.value.replace(REGEX_NEGATE_ONE, num => {
      return REGEX_TOKEN_REPLACEMENT.test(num) ? num.replace(REGEX_TOKEN_REPLACEMENT, m => { return '(-1*' + m + ')' }) : parseFloat(num) * -1
    })
  ...
  complement (value) {
    ...
    state.value = state.value.replace(REGEX_COMPLEMENT, num => {
      return REGEX_TOKEN_REPLACEMENT.test(num) ? num.replace(REGEX_TOKEN_REPLACEMENT, m => { return '(100% - ' + m + ')' }) : 100 - parseFloat(num)
    })
    return this.restoreTokens(state)
  },
  flipLength (value) {
