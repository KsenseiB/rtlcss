...
module.exports = function (options, plugins, hooks) {
  ...
  return {
    ...
    Declaration (node, { result }) {
      if (shouldProcess(node, result)) {
        ...
        if (!context.util.each(context.config.plugins,
          plugin => context.util.each(plugin.directives.value, directive => {
        ...
        context.util.each(context.config.plugins, plugin => context.util.each(plugin.processors, processor => {
    ...
    OnceExit (root, { result }) {
      ...
      Object.keys(toBeRenamed).forEach(key => {
